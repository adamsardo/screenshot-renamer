            ...
            if workspace.batches.isEmpty {
                ...
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 20) {
                        ForEach(workspace.batches.reversed()) { batch in
                            VStack(alignment: .leading, spacing: 10) {
                                HStack {
                                    ...
                                    Spacer()
                                    Button("Undo") { workspace.restore(batch, forward: false) }
                                        .disabled(workspace.busy || !batch.entries.contains { $0.location == .renamed && !$0.pending })
                                    Button("Redo / Retry") { workspace.restore(batch, forward: true) }
                                        .disabled(workspace.busy || !batch.entries.contains { $0.location == .original && !$0.pending })
                                }
